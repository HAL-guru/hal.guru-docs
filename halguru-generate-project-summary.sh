...
generate_git_information() {
  ...
  echo "" >> "$summary_file"
  echo "> The total lines of code in the 'hal.guru-docs' repository also include automatically generated Markdown files that are not tracked by Git." >> "$summary_file"
  echo "" >> "$summary_file"
}
...
generate_git_table_row() {
  ...
  local files_count=$(find . -type f | wc -l)
  local lines_count=$(
    find . -type f \( \
      -name "*.md" -o \
      -name "*.cs" -o \
